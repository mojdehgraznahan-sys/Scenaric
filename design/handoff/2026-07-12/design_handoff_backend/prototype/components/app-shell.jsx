  ...
  return (
    <div data-screen-label={"App · " + page} style={{ display: "flex", height: "100vh", overflow: "hidden", background: "#F5F5F5" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", marginLeft: 0 }}>
        {!["storyline", "narrative"].includes(page) && <TopBar page={page} navigate={navigate}/>}
...
function PageContainer({ page, navigate }) {
  switch (page) {
    case "projects":  return <window.PageProjects navigate={navigate}/>;
    ...
    default:           return <window.PageProjects navigate={navigate}/>;
  }
}
        ...
        <div style={{ padding: "11px 11px 4px" }}>
          ...
          {page !== "projects" && (
            <div style={{ marginTop: 7, padding: "6px 9px" }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#1E1B2E", letterSpacing: "-0.01em" }}>{store.project.name}</div>
...
function NavItem({ item, active, collapsed, onClick, groupStart, disabled }) {
  ...
  return (
    ...
    >
      <button
        ...
        style={{
          ...(collapsed ? { justifyContent: "center", padding: 0, width: 38, margin: "2px 7px" } : {}),
          ...(disabled ? { opacity: 0.4, cursor: "not-allowed", pointerEvents: collapsed ? "auto" : "none" } : {}),
        }}
      >
        {item.icon}
...
function TopBar({ page, navigate }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontWeight: 600, fontSize: 14, letterSpacing: "-0.01em" }}>{store.project.name}</div>
      {page !== "projects" && (
        <div style={{ display: "flex", alignItems: "center", gap: 4, marginLeft: 8, fontSize: 12, color: "#9CA3AF", fontFamily: "var(--font-mono)" }}>
          <span>/</span>
          <span>{titles[page] || "Scenaric"}</span>
        </div>
      )}
...
// Shared breadcrumb used by pages that render their own header instead of
// TopBar (Matrix, Storyline, Narrative) — kept visually identical to the
// "{Page} / {Project}" breadcrumb TopBar shows on Home / Knowledge Base, etc.
function PageBreadcrumb({ label }) {
  const store = window.FM.useStore();
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 2 }}>
      <span style={{ fontWeight: 600, fontSize: 14, letterSpacing: "-0.01em", color: "#1E1B2E" }}>{store.project.name}</span>
      <span style={{ fontSize: 12, color: "#9CA3AF", fontFamily: "var(--font-mono)" }}>/</span>
      <span style={{ fontSize: 12, color: "#9CA3AF", fontFamily: "var(--font-mono)" }}>{label}</span>
    </div>
  );
}
window.PageBreadcrumb = PageBreadcrumb;
